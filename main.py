...
import audioop  # noqa: F401 — loads our local shim before discord does
...
<title>Transcript — {channel.name} | Valora</title>
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Rajdhani:wght@700&display=swap');
:root{{--bg:#0d0f14;--s1:#13161e;--s2:#1a1e2a;--br:#1e2333;--bl:#00BFFF;--tx:#e0e6f0;--mu:#6b7590;--sg:#00e5a0;--ow:#FFD700;--bt:#5865F2}}
*{{box-sizing:border-box;margin:0;padding:0}}
body{{background:var(--bg);color:var(--tx);font-family:'Inter',sans-serif;font-size:14px;line-height:1.6}}
.hd{{background:linear-gradient(135deg,#0a0c14,#0d1220,#0a0c14);border-bottom:1px solid var(--br);padding:24px 40px;display:flex;align-items:center;gap:20px;position:sticky;top:0;z-index:99;backdrop-filter:blur(20px)}}
.hl{{width:60px;height:60px;border-radius:50%;border:2px solid var(--bl);box-shadow:0 0 18px rgba(0,191,255,.35)}}
.hi h1{{font-family:'Rajdhani',sans-serif;font-size:24px;color:var(--bl);letter-spacing:2px;text-shadow:0 0 18px rgba(0,191,255,.4)}}
.hi p{{color:var(--mu);font-size:12px;margin-top:2px}}
.hm{{margin-left:auto;display:flex;flex-direction:column;align-items:flex-end;gap:3px;font-size:11px;color:var(--mu)}}
.hm strong{{color:var(--tx)}}
.sb{{background:var(--s1);border-bottom:1px solid var(--br);padding:10px 40px;display:flex;gap:28px}}
.sl{{font-size:10px;color:var(--mu);text-transform:uppercase;letter-spacing:.8px}}
.sv{{font-size:14px;font-weight:600;color:var(--bl)}}
.ms{{max-width:880px;margin:0 auto;padding:20px 40px}}
.dd{{display:flex;align-items:center;gap:10px;margin:20px 0}}
.dd::before,.dd::after{{content:'';flex:1;height:1px;background:var(--br)}}
.dd span{{font-size:10px;color:var(--mu);text-transform:uppercase;letter-spacing:1px;padding:3px 10px;background:var(--s1);border-radius:20px;border:1px solid var(--br)}}
.mg{{display:flex;gap:12px;padding:5px 8px;border-radius:8px;margin:1px -8px;transition:background .12s}}
.mg:hover{{background:var(--s2)}}
.av{{width:38px;height:38px;border-radius:50%;flex-shrink:0;margin-top:2px;border:1px solid var(--br)}}
.avs{{width:38px;flex-shrink:0}}
.mc{{flex:1;min-width:0}}
.mh{{display:flex;align-items:center;gap:6px;margin-bottom:2px}}
.un{{font-weight:600;font-size:14px}}
.ts{{font-size:10px;color:var(--mu)}}
.badge{{font-size:9px;font-weight:700;padding:1px 5px;border-radius:3px;letter-spacing:.5px}}
.badge.staff{{background:rgba(0,229,160,.15);color:var(--sg);border:1px solid rgba(0,229,160,.3)}}
.badge.owner{{background:rgba(255,215,0,.15);color:var(--ow);border:1px solid rgba(255,215,0,.3)}}
.badge.bot{{background:rgba(88,101,242,.15);color:var(--bt);border:1px solid rgba(88,101,242,.3)}}
.mt{{color:#c9d1e0;word-break:break-word}}
.mt a{{color:var(--bl);text-decoration:none}}
.mt code{{background:#1e2333;padding:1px 4px;border-radius:3px;font-family:monospace;font-size:12px}}
.att-img{{max-width:380px;max-height:280px;border-radius:8px;margin-top:6px;display:block;border:1px solid var(--br)}}
.att-file{{display:inline-block;margin-top:5px;padding:5px 10px;background:var(--s2);border:1px solid var(--br);border-radius:5px;color:var(--bl);text-decoration:none;font-size:12px}}
.emb{{margin-top:6px;background:var(--s2);border-left:4px solid var(--bl);border-radius:4px;padding:8px 12px;max-width:500px}}
.et{{font-weight:600;color:var(--tx);margin-bottom:3px;font-size:13px}}
.ed{{color:var(--mu);font-size:12px;white-space:pre-line}}
.ft{{text-align:center;padding:36px;border-top:1px solid var(--br);color:var(--mu);font-size:11px;margin-top:36px}}
.ft a{{color:var(--bl);text-decoration:none}}
.fl{{font-family:'Rajdhani',sans-serif;font-size:20px;font-weight:700;color:var(--bl);letter-spacing:2px;margin-bottom:6px}}
</style>
</head>
<body>
<div class="hd">
  {logo_html}
  <div class="hi"><h1>VALORA STORE</h1><p>{cat["emoji"]} {cat["label"]} • #{channel.name}</p></div>
  <div class="hm"><span>Generated: <strong>{datetime.now(timezone.utc).strftime("%d/%m/%Y %H:%M")} UTC</strong></span><span>Server: <strong>{guild.name}</strong></span></div>
</div>
<div class="sb">
  <div class="stat"><div class="sl">Messages</div><div class="sv">{len(messages)}</div></div>
  <div class="stat"><div class="sl">Channel</div><div class="sv">#{channel.name}</div></div>
  <div class="stat"><div class="sl">Category</div><div class="sv">{cat["label"]}</div></div>
</div>
<div class="ms">
  <div class="dd"><span>Ticket Opened</span></div>
  {msgs_html}
  <div class="dd"><span>Ticket Closed</span></div>
</div>
<div class="ft">
  <div class="fl">💎 VALORA</div>
...
class StoreView(discord.ui.View):
    def __init__(self):
        ...
        self.add_item(discord.ui.Button(
            label="Visit Store", style=discord.ButtonStyle.link,
...
async def cmd_add(interaction: discord.Interaction, user: discord.Member):
    ...
    await interaction.response.send_message(
        embed=discord.Embed(description=f"✅ {user.mention} added to the ticket.", color=discord.Color.green())
    )
...
async def cmd_remove(interaction: discord.Interaction, user: discord.Member):
    ...
    await interaction.response.send_message(
        embed=discord.Embed(description=f"✅ {user.mention} removed from the ticket.", color=discord.Color.red())
    )
